...
def bfs(grid, start, target, wall):
    ...
    normal_bomb, mega_bomb = 0,0
    for k, v in my_allies.items():
        if k == 'M':
            normal_bomb, mega_bomb = v[2], v[3]

    while queue:
        (r, c), actions = queue.popleft()

        # ----------사거리를 고려하여 사거리 3안에 타겟이 있으면 발사 ----------
        # ----------포탄의 개수를 고려하여 더 많이 있는 포탄을 발사 ----------
            ...
            for _ in range(3):
                ...
                if 0 <= fr < rows and 0 <= fc < cols:
                    if grid[fr][fc] == wall:
                        ...
                        if normal_bomb >= mega_bomb:
                            return actions + [FIRE_CMDS[d]]
                        elif normal_bomb < mega_bomb:
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            # ----------물을 지나가지 못하는 로직 추가----------
            # ----------가는 길에 나무가 있으면 제거----------
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] not in(wall, 'W', 'T') and (nr, nc) not in visited:
                visited.add((nr, nc))
                queue.append(((nr, nc), actions + [MOVE_CMDS[d]]))
            
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] == 'T':
                if normal_bomb >= mega_bomb:
                    queue.append(((nr, nc), actions + [FIRE_CMDS[d]]))
                else:
                    queue.append(((nr, nc), actions + [MEGA_FIRE_CMDS[d]]))

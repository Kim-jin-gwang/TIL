...
def get_enemy_targets():
    ...
    targets.sort(key=lambda x: (x[2] != 'X', x[0]))
...
def bfs_to_target(start, target_pos):
    ...
    for fp in supply_positions:
        ...
        for dr, dc in DIRS:
            ...
            if 0 <= ar < len(map_data) and 0 <= ac < len(map_data[0]):
                if map_data[ar][ac] not in {'R', 'W', 'T', 'E1', 'E2', 'E3', 'X', 'F'}:
...
##############################
actions    = []
MEGA_TARGET = 4   # 전투 시작 전 목표 메가 포탄 개수
...
while game_data is not None:
    ...
    # ============================================================
    # [보급 모드] 메가 포탄 MEGA_TARGET개 모을 때까지
    # ============================================================
    if mega_bomb < MEGA_TARGET:

        # 1-1: 보급 시설 인접 + 암호문 존재 → 해독 시도
        if codes and is_adjacent_to_supply(my_pos):
            cmd = try_decode_and_submit(codes[0].strip())
            if cmd:
                output = cmd
                print(f'[SUPPLY] 암호 해독 시도: {output}')

        # 1-2: 아직 인접 안 했으면 보급 시설로 이동
        if output is None:
            if not actions:
                actions = move_to_supply(my_pos)
                print(f'[SUPPLY] 보급 시설로 이동 중... ({len(actions)}턴)')

    # ============================================================
    # [전투 모드] 메가 포탄 MEGA_TARGET개 달성 후
    # ============================================================
    else:

        # 2-1: 현재 위치에서 사격 가능하면 즉시 발사
        ...
        for t_pos, t_key in targets:
            ...
            if shot_cmd:
                output = shot_cmd
                actions = []  # 사격 후 경로 재계획
                print(f'[BATTLE] 사격: {output} → {t_key}')
                break

        # 2-2: 사격 불가 → 사격 위치로 BFS 이동
        if output is None:
            if not actions:
                targets = get_enemy_targets()
                if targets:
                    best_target_pos, _ = targets[0]
                    actions = bfs_adjacent_to_target(my_pos, best_target_pos)
                    if not actions:
                        actions = bfs_to_target(my_pos, best_target_pos)
                    print(f'[BATTLE] 사격 위치로 이동 중... ({len(actions)}턴)')

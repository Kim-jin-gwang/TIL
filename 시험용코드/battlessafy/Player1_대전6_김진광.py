...
def bfs_flanking(start, target_pos, avoid_danger=True):
    ...
    # 다익스트라 방식 (위험 구역 비용 높게)
    # deque 대신 우선순위 없이 BFS + 위험 구역 우선 회피
    queue = deque([(start, [], False)])  # (위치, 경로, 위험구역_통과_여부)
    visited = {start: False}  # 위험구역 없이 방문했는지

    best_safe = None    # 안전 경로
    best_unsafe = None  # 불가피한 경우 위험 경로
    ...
    while queue:
        ...
        if (r, c) == target_pos:
            if not in_danger:
                return path  # 안전 경로 즉시 반환
            elif best_unsafe is None:
                best_unsafe = path
            continue
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            prev_danger = visited.get((nr, nc), None)

            # 안전하게 방문 가능하거나, 이전에 위험하게 방문한 곳을 안전하게 재방문
            if prev_danger is None or (prev_danger and not new_danger):

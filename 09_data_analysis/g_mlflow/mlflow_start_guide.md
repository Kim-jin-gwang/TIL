...
> UI 확인을 마쳤다면, 반드시 `Ctrl + C` 등으로 해당 UI서버를 종료하고 **다음 단계(7번)**로 진행하세요.  
> 단순 UI 확인은 서버를 띄우는 것과 다르므로 확인용입니다.
...
## 8. 실험 생성
터미널에서 실행할 경우 다른 터미널에서 트래킹 환경변수 선언 후 실행:
...
```bash
cd ~/mlflow_project/src/chapter2/

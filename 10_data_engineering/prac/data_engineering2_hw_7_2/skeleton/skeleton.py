...
def choose_branch(**kwargs):
    ...
    return f"task_{value}"  # 실행할 Task ID 반환

# DAG (Directed Acyclic Graph) 정의
with DAG(
    dag_id="simple_branch_operator_example",  # DAG의 고유한 ID 설정
    start_date=pendulum.datetime(2026, 5, 19, tz="Asia/Seoul"),  # DAG 시작 날짜 및 시간대 설정
    ...
    end_task = BashOperator(
        ...
        trigger_rule="none_failed_min_one_success"  # 실패하거나 건너뛴 Task가 없을 경우 실행
    )

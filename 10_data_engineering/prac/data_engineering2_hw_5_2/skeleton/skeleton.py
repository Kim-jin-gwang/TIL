...
with DAG(
    ...
    start_date=pendulum.datetime(2026, 5, 19, tz="Asia/Seoul"),  # DAG의 시작 날짜 (2026년 5월 19일)
) as dag:

    # DAG 안에서 실행할 태스크(Task) 정의
    # _____ 는 실제 실행할 작업이 없는 "빈" 태스크를 의미함
    start = EmptyOperator(task_id="start_task")  # 시작 지점 역할
    end = EmptyOperator(task_id="end_task")  # 끝 지점 역할

# 실습 힌트
# - 목표: Kafka stock_trades 토픽을 PyFlink SQL 소스로 읽고 stock_stats 토픽으로 집계 결과를 보냅니다.
# - 먼저 Kafka와 kafka_producer.py를 실행해 입력 데이터가 들어오게 하세요.
# - TODO은 Kafka connector/format, StatementSet 생성, SUM(volume), AVG(price), add_insert_sql/execute 흐름입니다.
# - Kafka 주소는 로컬 실습 기준 localhost:9092입니다. Docker 내부 실행이면 kafka:9092로 바꾸세요.

# TODO 안내
# - Kafka source/sink 테이블의 connector와 format에는 Kafka와 JSON 관련 값을 넣습니다.
# - 집계 SQL은 volume 합계와 price 평균을 계산합니다.
# - StatementSet에는 INSERT INTO kafka_sink SELECT ... 쿼리를 등록하고 실행합니다.
...
def monitor_job(job_client):
    ...
    try:
        ...
        print("kafka-console-consumer.sh --topic stock_stats --bootstrap-server localhost:9092 --from-beginning")

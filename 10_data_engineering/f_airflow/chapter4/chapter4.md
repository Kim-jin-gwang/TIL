...
# 예시일 뿐이니 실제 사용은 제공한 파일을 기반으로 진행합니다.
# 해당 파일을 생략하고 gitlab에서 docker-compose 파일을 받아 실행합니다.
...
---
# 기존 파일에서 변경된 사항(학습용 docker-compose.yaml)

## 3. docker-compose.yaml: PostgreSQL 포트 설정
...
> 우리가 5433로 접속하면서 Docker 컨테이너 상에서 5432 port로 띄워놓는 형태
> Airflow 메타데이터 DB는 `localhost:5433`로 접속합니다.
...
`docker-compose.yaml`의 환경변수에 아래 추가:
...
- `docker-compose-postgres.yaml`로 `postgres-db` 서비스를 실행합니다.  
- 기본 설정: `user=ssafyuser`, `password=ssafy`, `db=ssafydb`, 컨테이너 내부 포트 `5432`, 호스트 포트 `5442`  
- 로컬 PC에서는 `localhost:5442`로 접속하고, Airflow 컨테이너 내부에서는 `postgres-db:5432`로 접속합니다.  
...
1. **Airflow Web UI → Admin → Connections → + 버튼**
...
# Spark 연계를 위한 docker-compose 변경사항 정리

`docker-compose-airflow-spark.yaml` 파일을, 스파크와 연계해서 활용하기 위해서 추가된 코드와 변경 이유를 정리한 문서입니다.
해당되는 새로운 docker-compose 파일은 디렉토리에 업로드되어 있으니 spark와 연계 사용을 위해서는 이 파일을 사용하기를 권장합니다.

---

## 1. Airflow Image → Dockerfile Build 변경
- 해당 파일은 `ssafy_airflow/docker-compose-airflow-spark.yaml`로 제공되어 있습니다.
- 파일명에 맞게 `docker compose -f docker-compose-airflow-spark.yaml up -d --build`를 통해 컨테이너들을 띄울 수 있습니다.
- `--build`를 붙여야 `Dockerfile.airflow` 또는 `Dockerfile.spark` 변경사항이 이미지에 다시 반영됩니다.
...
- Airflow 이미지에는 `Dockerfile.airflow`에서 OpenJDK 17을 설치하고, Spark 이미지는 `apache/spark:3.5.4-java17-python3` 기반으로 Java 17을 사용합니다.

---

## 3. 볼륨 추가 (scripts, data, output)
...
변경 사항을 적용하기 전에 `data`, `output`, `dags/scripts` 디렉토리가 없으면 먼저 생성하세요.

```bash
mkdir -p ./data ./output ./dags/scripts
...
**추가 이유:**
- 모든 서비스 간 안정적 통신을 위해 미리 생성한 `shared-net` 네트워크에 연결하여 컨테이너간 통신 이슈를 없애기 위함.
- 컨테이너 DNS 이름(`spark-master` 등)으로 쉽게 통신 가능.

---

## 6. Spark Master/Worker 서비스 추가
...
## 8. Spark Job 제출을 위한 Client

- Airflow가 SparkSubmitOperator 또는 Docker exec/BashOperator를 통해 Spark Job을 제출하려면, Airflow 컨테이너 안에서 `spark-submit` 명령어를 실행할 수 있어야 합니다.
- 이를 위해 Dockerfile.airflow에서 Spark client를 설치합니다.
- 현재 `Dockerfile.airflow`는 `spark-3.5.4-bin-hadoop3.tgz`를 빌드 컨텍스트에서 `COPY`해서 설치하므로, 해당 tgz 파일이 `docker-compose-airflow-spark.yaml`과 같은 디렉토리에 있어야 합니다.
- 실제 운영 환경에서는 Airflow에 Spark를 설치하지 않고도, KubernetesOperator, YARN 등 클러스터 매니저 등을 통해 Job을 제출할 수 있습니다.
- 이렇게 되면 Airflow는 오케스트레이션만 담당하고, 실제 연산과 리소스 관리는 클러스터가 수행합니다.
- Airflow → Spark Job 제출 구조를 직관적으로 이해할 수 있도록, Airflow 컨테이너 안에 Spark client를 설치하고 SparkSubmitOperator를 직접 사용하도록 구성합니다.

---

## 9. Airflow UI에서 Spark Connection 추가

`SparkSubmitOperator(conn_id="spark_default")`가 실행되려면, **Connection ID = `spark_default`** 가 Airflow에 미리 등록되어야 합니다.
...
> 현재 Spark Master UI는 [http://localhost:8083](http://localhost:8083)에서 확인할 수 있습니다.

---

## 10. SparkSubmitOperator 실행 및 로그 확인

- 예제 DAG: `dags/4_spark_submit_example.py`
- Spark 애플리케이션 경로: `/opt/airflow/dags/scripts/spark_wordcount.py`
- Spark Master 주소: `spark://spark-master:7077`
...
- DAG 실행 중 오류가 발생하면 Airflow Web UI의 Task 로그에서 실패 원인을 확인합니다.
- 로컬 `logs/` 폴더에도 DAG별 실행 로그가 저장됩니다.

...
Docker 기반 Hadoop 클러스터에서 MapReduce 개념을 이해하고, Java 내장 예제 및 Python 기반 사용자 정의 MapReduce까지 실습하는 과정을 정리

실습 명령은 별도 표시가 없으면 `namenode` 컨테이너 안에서 실행합니다.
...
(Docker 이미지에는 적용되어 있음)

Hadoop은 컨테이너 내부의 `/usr/local/hadoop`에 설치되어 있습니다.
...
설정을 직접 수정했다면 관련 컨테이너의 YARN 데몬을 재시작해야 반영됩니다.

```bash
docker exec namenode bash -lc "yarn --daemon stop resourcemanager; yarn --daemon start resourcemanager"
docker exec datanode1 bash -lc "yarn --daemon stop nodemanager; yarn --daemon start nodemanager"
docker exec datanode2 bash -lc "yarn --daemon stop nodemanager; yarn --daemon start nodemanager"
```
...
```bash
ls /usr/local/hadoop/share/hadoop/mapreduce/hadoop-mapreduce-client-app-*.jar
```

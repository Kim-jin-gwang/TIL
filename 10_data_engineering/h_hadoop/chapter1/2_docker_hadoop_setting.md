...
- 모든 노드는 외부 Docker 네트워크 `shared-net`을 공유하며 내부적으로 서로 통신합니다.
- Elasticsearch와 별도 Compose로 함께 실행하려면 최초 1회 `docker network create shared-net`을 실행합니다.
...
## 설명
- Hadoop 3.x는 **Java 11**을 권장 → JAVA_HOME 설정 필수.
...
### 설명
- MapReduce v2는 **YARN 기반**으로 실행되기 때문에 필수 설정.
...
- CapacityScheduler는 Docker 실습 환경에서 단일 `default` 큐를 사용해 작업을 안정적으로 실행.

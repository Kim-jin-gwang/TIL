...
# - 로컬 → HDFS 방향의 파일 이동
hadoop fs -put -f test.txt /user/local/hadoop_data
...
# - HDFS → 로컬 방향의 파일 복사
hadoop fs -get -f /user/local/hadoop_data/test.txt /tmp/hdfs_test/hadoop_test.txt

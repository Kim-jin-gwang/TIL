...
## 실행 위치
호스트에서 `docker exec -it namenode bash`로 `namenode` 컨테이너에 접속한 뒤, skeleton 디렉터리에서 실행합니다. 입력 CSV는 `financial_transactions_sample.csv` 경로를 사용합니다.
...
- image1: HDFS 입력 CSV 업로드 확인 화면
- image2: Mapper 단독 실행 테스트 결과 화면
- image3: Reducer 단독 실행 테스트 결과 화면
- image4: Hadoop Streaming 결과 상위 10개 출력 화면

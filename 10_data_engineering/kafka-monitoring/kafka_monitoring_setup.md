...
#### Kafka 모니터링 지표 의미와 활용

##### 1. MessagesInPerSec

* 각 토픽의 모든 파티션에서 들어오는 메시지 수를 합산한 **토픽 전체 초당 메시지 처리량**
  * 토픽 트래픽 패턴 분석 및 피크 시간 확인
  * 토픽 과부하 탐지 및 브로커 부하 예측
  * 프로듀서/컨슈머 튜닝

##### 2. BytesInPerSec

* 각 토픽의 모든 파티션에서 들어오는 바이트 수를 합산한 **토픽 전체 초당 수신량**
  * 네트워크 사용량 모니터링 및 대역폭 계획
  * 메시지 크기 변화 감지 → 이상 패턴 탐지
  * 브로커 리소스 계획 (디스크 I/O, 메모리)

##### 3. BytesOutPerSec

* 각 토픽의 모든 파티션에서 나가는 바이트 수를 합산한 **토픽 전체 초당 송신량**
  * 컨슈머 소비 속도 모니터링 → 백로그 탐지
  * 네트워크 병목 확인 → 토픽별 소비자 튜닝
  * SLA 모니터링 → 지연 알람 설정

##### 4. 종합적 의미

* 세 지표를 통해 **토픽 단위 메시지/데이터 흐름 전체 상황**을 파악
* 이상치 감지 및 운영 안정성 확보
...
## 11. Docker Compose (Kafka 모니터링 포함)
